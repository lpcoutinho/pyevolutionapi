...
def basic_example():
    ...
    try:
        ...
        instance_response = client.instance.create(instance_name="my-whatsapp-bot", qrcode=True)
...
async def async_example():
    """Async example."""
    client = EvolutionClient(base_url="http://localhost:8080", api_key="your-api-key-here")

    try:
        async with client:  # Use as async context manager
            # Create instance
            instance_response = await client.instance.acreate(
                instance_name="async-bot", qrcode=True
            )
            ...
            tasks = [
                client.messages.asend_text(
                    instance="async-bot", number="5511999999999", text=f"Async message {i}"
                )
                for i in range(1, 4)
            ]
