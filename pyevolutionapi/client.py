...
class EvolutionClient:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.base_url = (base_url or os.getenv("EVOLUTION_BASE_URL", "http://localhost:8080")).rstrip("/")
        ...
        self.timeout = timeout or float(os.getenv("EVOLUTION_REQUEST_TIMEOUT", str(self.DEFAULT_TIMEOUT)))
        self.max_retries = max_retries or int(os.getenv("EVOLUTION_MAX_RETRIES", str(self.DEFAULT_MAX_RETRIES)))
    ...
    def request(
        ...
        **kwargs
        ...
        try:
            ...
            raise EvolutionTimeoutError(f"Request timed out: {e}")
        except httpx.ConnectError as e:
            raise EvolutionConnectionError(f"Connection failed: {e}")
        except Exception as e:
            raise EvolutionAPIError(f"Request failed: {e}")
    ...
    def _handle_response(self, response: httpx.Response) -> None:
        ...
        if response.status_code == 401:
            ...
            raise EvolutionAPIError(f"Server error: {message}", status_code=response.status_code, response_data=error_data)
        else:
            raise EvolutionAPIError(message, status_code=response.status_code, response_data=error_data)
